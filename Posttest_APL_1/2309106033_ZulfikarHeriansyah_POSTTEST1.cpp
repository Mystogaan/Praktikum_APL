...
setTextColor(0);
    ...
    do { // perulangan agar bisa meminta input yes or no kepada user
        clear();
        while(true){//perulangan untuk error handling jika input user tidak sesuai
        setTextColor(33);
        cout << "=====================" << endl;//header main
        ...
        cin >> pilihan;
        if(cin.fail()){//error handling ketika input tidak sesuai
            clear();//fungsi membersihkan terminal
            setTextColor(31);
        ...
        }else if(pilihan < 1 || pilihan > 5){//error handling agar inputan hanya di range 1 - 5
            setTextColor(31);
            clear();
        ...
        switch(pilihan) {
            case 1:
            clear();
            for (int i=0;i<3;i++){//perulangan jika user salah memasukkan tipe data 
                ...
                if (cin.fail()) {
                    ...
                    cin.ignore(numeric_limits<streamsize>::max(), '\n');//numeric limit untuk memberikann range dari tipe data yang diminta dan cin.ignore untuk membersihkan buffer input
                        if(i==2){
                        setTextColor(31);
                        cout << "MAAF KESEMPATAN HABIS\n"; //output saat sudah 3 kali menginput 
                        setTextColor(0);
                        break;//keluar dari perulanngan
                }   //kondisi jika input salah 
                    setTextColor(31);
            ...
            }   //fungsi untuk mengkonversi rupiah
            ...
            while(true){//perulangan agar input user hanya y/n
    ...
    } while(answer == "y" || answer == "Y");//kondisi untuk melakukan perulangan
    return 0;
}
