...
void menuDua(){
    for (int i=0;i<jumlahNovelDitambahkan;i++){ //menampilkan data novel
        cout << i+1 <<". " << "JUDUL : " << dataNovel[i].judul << endl << "GENRE : " << dataNovel[i].genre << endl << "TAHUN TERBIT : " << dataNovel[i].detail.tahunTerbit << endl << "NAMA PENERBIT : " << dataNovel[i].detail.penerbit << endl << endl;
    }
}
void menuTiga(){
...
void menuEmpat(){
    ...
    if ((jumlahNovelDitambahkan) > 0){
        ...
        if(delNovel<(jumlahNovelDitambahkan) && delNovel>=0){
            ...
            jumlahNovelDitambahkan = jumlahNovelDitambahkan - 1;
            clear();
            setTextColor(32);
